...
def gradient_descent(gradient, start, learn_rate=0.1, n_iter=200, tolerance=1e-6):
...
def gradient_descent_real(gradient, start, learn_rate=0.1, n_iter=200, tolerance=1e-6):
    vector = np.array(start, dtype=gradient.dtype)
    history = []
    for _ in tqdm(range(n_iter)):
...
def gradient_descent_parallel(
    gradient, start, learn_rate=0.1, n_iter=200, tolerance=1e-6, num_processes=4
):
    ...
    chunks = [
        start[:, i * chunk_size : (i + 1) * chunk_size] for i in range(num_processes)
    ]
    for i in chunks:
        print(i.shape)
