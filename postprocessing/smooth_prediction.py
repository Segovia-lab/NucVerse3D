...
class smooth_predict:
    ...
    def __init__(
        ...
        # SANITY CHECKS
        assert len(input_img.shape) == 4
        "Data must be in z,y,x,channels format"
        for rot_couple in rot_axes:
            assert len(rot_couple) == 2
            "Two axes are required for defining a rotation plane"
            assert all(dim < 4 for dim in rot_couple)
            "Rotation plane does not exist"
            assert rot_couple[0] != rot_couple[1]
            "Rotation plane must be defined by two different axes"
        ...
        self.padded_out_shape = list(self.padded_original.shape[:-1]) + [
            self.nb_classes
        ]
    ...
    def _pad_img(self, in_img):
        ...
        assert self.window_size[0] % self.subdivisions == 0
        "window size must be divisible by subdivisions"
    ...
    def _rot_save(self, im, flip, r_list):
        ...
        for i, ax in enumerate(tqdm(self.rot_axes)):
            for n_rot in range(3):
                ...
                fpath = self.tmp_path.joinpath(
                    "rot_{}_{}_{}.npy".format("".join(tuple(map(str, flip))), i, n_rot)
                )
                np.save(file=fpath, arr=np.rot90(np.array(im), k=n_rot, axes=ax))
...
class single_view_predictor:
    ...
    def _predict_batch(self):
        ...
        for i, prediction in enumerate(pred_batch):
            ...
            z, y, x = pos_l[i]
            self.pred_img[
                z : z + window_size, y : y + window_size, x : x + window_size
            ] = (
                self.pred_img[
                    z : z + window_size, y : y + window_size, x : x + window_size
                ]
            ...
            for y in y_points:
                for x in x_points:
                    ...
                    assert patch.shape == (
                        ...
                        self.window_size,
                        self.window_size,
                        self.padded_img.shape[-1],
                    )
                    "Padded image should contain an integer number of windows, something's wrong with padding"
