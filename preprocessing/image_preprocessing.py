...
def create_psf(size=config.PSF_SIZE, sigma=config.PSF_SIGMA):
    """
...
def correct_intensity_per_slice_uneven(
    image_3d,
    sigma=config.INTENSITY_SIGMA,
    threshold=config.INTENSITY_THRESHOLD,
    ds=config.INTENSITY_DOWNSCALE,
):
    img_ds = downscale_local_mean(image_3d, ds).astype(np.float32)
    gauss = gaussian_filter(img_ds, sigma=sigma)
    gauss = resize(gauss, image_3d.shape, order=1)
    gauss[gauss < threshold] = threshold
    mean_slice = np.mean(image_3d)
    corrected = image_3d * mean_slice / gauss
    return corrected


def normalize_image(
    image, p_lower=config.NORMALIZE_P_LOWER, p_upper=config.NORMALIZE_P_UPPER
):
    ...
    normalized_image = exposure.rescale_intensity(
        image, in_range=(lower_bound, upper_bound), out_range=(0, 1)
    )
...
def preprocessing(img):
    ...
    return norm_img.astype(config.FLOAT_DTYPE)

def preprocessing_simple(img):
...
def optical_flow_task_middle(image0, image1, order=3):
    ...
    image1_warp = warp(
        image1,
        np.array([row_coords + v / 2, col_coords + u / 2]),
        mode="edge",
        order=order,
    )
    image0_warp = warp(
...
def aligned_image(img, order=3):
    ...
    img_impar = np.stack(
        [img[:, i, :] for i in range(len(img[0])) if i % 2 != 0], axis=1
    )

    img_par_rescale = resize(
        img_par, (img_par.shape[0], img_par.shape[1] * 2, img_par.shape[2]), order=3
    )
    img_impar_rescale = resize(
        img_impar,
        (img_impar.shape[0], img_impar.shape[1] * 2, img_impar.shape[2]),
        order=3,
    )
    ...
    with joblib_progress("Alineacion:", total=len(img_impar_rescale)):
        results = Parallel(n_jobs=num_jobs)(
            delayed(optical_flow_task_middle)(
                img_par_rescale[i], img_impar_rescale[i], order
            )
            for i in range(len(img_impar_rescale))
        )
...
def aligned_image_simple(img, order=3):
    ...
    img_par_rescale = resize(
        img_par, (img_par.shape[0], img_par.shape[1] * 2, img_par.shape[2]), order=order
    )
    img_impar_rescale = resize(
        img_impar,
        (img_impar.shape[0], img_impar.shape[1] * 2, img_impar.shape[2]),
    ...
    return final.astype(config.FLOAT_DTYPE)
