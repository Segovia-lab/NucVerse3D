...
def reshape(inputs, outputs, targets, orig_img):
    ...
    end_h = start_h + h
    return inputs[:,:,start_h:end_h,start_w:end_w,start_d:end_d], \
...
def transpose3D(X, y):
    ...
    X_trans, y_trans = np.transpose(X, (p[0], p[1], p[2], 3)), np.transpose(y, (p[0], p[1], p[2]))
...
def random_decisions(N):
    ...
    decisions = np.zeros((N, 8)) # 5 is number of aug techniques to combine (patch extraction is not used here)
    for n in range(N):
