...
def encode_norm(vol_gt, vox_threshold = 100):
    ...
    if(len(labels)!=0):
        ...
        z1,y1,x1,z2,y2,x2 = bbox[:,0], bbox[:,1], bbox[:,2], bbox[:,3], bbox[:,4], bbox[:,5]
    
        # Obtener cada máscara de cada núcleo
        mask_i = [vol_gt[z1[i]:z2[i],y1[i]:y2[i],x1[i]:x2[i]]==labels[i] for i in range(len(z1))]
    
        # Obtener dimensiones de la bonding box de cada objeto
        h, w, d = bbox[:,3]-bbox[:,0], bbox[:,4]-bbox[:,1], bbox[:,5]-bbox[:,2]
        ...
        for i in range(len(z1)):
            grad_z_concat =  np.concatenate([np.linspace(centroids[i,0]-z1[i],0, centroids[i,0]-z1[i]+1),
                                            np.linspace(0,-(z2[i]-centroids[i,0]-1), z2[i]-centroids[i,0])[1:]])
            max_grad_z = np.max(np.abs(grad_z_concat)) + 1e-10
            grad_z_concat_norm = (grad_z_concat/max_grad_z).reshape(-1,1,1)
            
            grad_y_concat = np.concatenate([np.linspace(centroids[i,1]-y1[i],0, centroids[i,1]-y1[i]+1),
                                            np.linspace(0,-(y2[i]-centroids[i,1]-1), y2[i]-centroids[i,1])[1:]])
            max_grad_y = np.max(np.abs(grad_y_concat)) + 1e-10
            grad_y_concat_norm = (grad_y_concat/max_grad_y).reshape(1,-1,1)
            
            grad_x_concat = np.concatenate([np.linspace(centroids[i,2]-x1[i],0, centroids[i,2]-x1[i]+1),
                                            np.linspace(0,-(x2[i]-centroids[i,2]-1), x2[i]-centroids[i,2])[1:]])
        ...
        target_vector = np.stack([grad_z.astype(np.float32), grad_y.astype(np.float32), grad_x.astype(np.float32)], axis = -1)
...
def calc_vote_image(centroid_vectors: np.array, f):
    ...
    size = np.array(np.array((height, width, depth), dtype='float32') * ((1/f), (1/f), (1/f)), dtype='int16')
