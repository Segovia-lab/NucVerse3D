...
@tf.keras.saving.register_keras_serializable(name="mse_loss")
...
@tf.keras.saving.register_keras_serializable(name="bce_loss")
def bce_loss(y_true, y_pred):
    loss = tf.keras.losses.binary_crossentropy(y_true, y_pred,)
    loss = tf.identity(loss, name = "bce_loss")
    return loss

@tf.keras.saving.register_keras_serializable(name="cce_weights_loss")
...
@tf.keras.saving.register_keras_serializable(name="total_loss")
...
@tf.keras.saving.register_keras_serializable(name="categorical_total_loss")
...
class FocalTverskyLoss(tf.keras.losses.Loss):
    def __init__(self, alpha=0.8, gamma=2, smooth=1, alpha_tversky=0.3, beta_tversky=0.7, smooth_tversky=1):
        super(FocalTverskyLoss, self).__init__()
        self.focal_loss = FocalLoss(alpha=alpha, gamma=gamma, smooth=smooth)
        self.tversky_loss = TverskyLoss(alpha=alpha_tversky, beta=beta_tversky, smooth=smooth_tversky)
