...
def conv_block_simple(x, filter_size, size, dropout, batch_norm=False):
    conv = layers.Conv3D(size, (filter_size, filter_size, filter_size), padding="same")(x)
    if batch_norm is True:
        conv = layers.BatchNormalization(axis=-1)(conv)
    conv = layers.Activation("relu")(conv)
...
def Attention_ResUNet_3D(input_shape, n_classes_1=1, n_classes_2 = 1, dropout_rate=0.2, batch_norm=True):
    ...
    pre_output_1 = conv_block_simple(up_conv_128, 32)
    conv_final = layers.Conv3D(n_classes_1, kernel_size=(1, 1, 1), padding="same")(pre_output_1)
    conv_final = layers.BatchNormalization(axis=axis)(conv_final)
    output_1 = layers.Activation(activation)(conv_final)

    pre_output_2 = conv_block_simple(up_conv_128, 32)
    output_2 = layers.Conv3D(3, kernel_size=(1, 1, 1), padding="same")(pre_output_2)
