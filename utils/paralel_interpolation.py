...
def linear_interp_vol(img, output_shape, patch_size, step):
    ...
    for z in range(len(img)):
        for j in range(len(img[0])):           
            merged_x.append(interp_X(img, output_shape, patch_size, step, overlap, z, j))
    merged_x = np.reshape(merged_x,(img.shape[0],img.shape[1],patch_size,patch_size,output_shape[-1]))
    
    # Interpolacion en Y
    merged_y = []
    for z in range(len(img)):
        merged_y.append(interp_Y(img, output_shape, patch_size, step, overlap, z, merged_x))
