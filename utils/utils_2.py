...
def interp_X(img, output_shape, patch_size, step, overlap, z, j, k):
    ...
    for i in range(len(img[0,0])):
        ...
        if(np.logical_and(img1[:,:,step*i:step*i+patch_size], img2).sum()):
            ...
            merged[:,:,step*i:step*i+overlap] = (1 - mask) * overlap1 + mask * overlap2
            merged[:,:,step*i+overlap:step*i+patch_size] = img2[:,:,overlap:]
        else:
...
def interp_Y(img, output_shape, patch_size, step, overlap, z, k, merged_x):
    ...
    for i in range(len(img[0])):
        ...
        if(np.logical_and(img1[:,step*i:step*i+patch_size,:], img2).sum()):
            ...
            mask = np.expand_dims(mask, -1)
            mask = np.repeat(mask, k, axis=-1)
            merged_xy[:,step*i:step*i+overlap,:] = (1 - mask) * overlap1 + mask * overlap2
            merged_xy[:,step*i+overlap:step*i+patch_size,:] = img2[:,overlap:,:]
        else:
...
def linear_interp_vol(img, output_shape, patch_size, step, k):
    ...
    for z in tqdm(range(len(img))):
        for j in range(len(img[0])):           
            merged_x.append(interp_X(img, output_shape, patch_size, step, overlap, z, j, k))
    merged_x = np.reshape(merged_x,(img.shape[0],img.shape[1],patch_size,patch_size,output_shape[-1], k))
    
    # Interpolacion en Y
    print("Interpolacion en Y")
    merged_y = []
    for z in tqdm(range(len(img))):
        merged_y.append(interp_Y(img, output_shape, patch_size, step, overlap, z, k, merged_x))
    ...
    for i in tqdm(range(len(img))):
        ...
        if(np.logical_and(img1[step*i:step*i+patch_size,:,:], img2).sum()):
            ...
            mask = np.expand_dims(mask, -1)
            mask = np.repeat(mask, k, axis=-1)
            merged_xyz[step*i:step*i+overlap,:,:] = (1 - mask) * overlap1 + mask * overlap2
            merged_xyz[step*i+overlap:step*i+patch_size,:,:] = img2[overlap:,:,:]
        else:
...
def pad_img(img, patch_size, overlap, mode = 'constant'):
    ...
    img_shape = img.shape
    img_pad = np.pad(img,pad_width = ((0,LS_img[0]-img_shape[0]),(0,LS_img[1]-img_shape[1]),(0,LS_img[2]-img_shape[2])), mode = mode)
    return img_pad
